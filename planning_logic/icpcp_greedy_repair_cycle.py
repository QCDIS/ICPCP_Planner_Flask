...
def dumpJSON(start, end):
    ...
    print
    "        { \"order\":", str(G.node[end]["order"]) + ","
    print
    "          \"name\":", "\"" + G.node[end]["name"]
    ...
    for (u, v) in G.edges():
        nedge += 1
        print
        "        { \"source\":", "\"" + G.node[u]["name"] + "\","
        print
        "          \"target\":", "\"" + G.node[v]["name"] + "\","
        print
        "          \"throughput\":", str(G[u][v]["throughput"]) + ","
        print
        "          \"inpath\": 0"
        if nedge < num_edges:
            print
...
def graphCheckEST():
    for n in xrange(0, number_of_nodes):
        ...
        if maxest > deadline:
            ...
            print
            "\n**** Wrong EST: " + "EST(" + G.node[n]["name"] + ")=" + str(
                G.node[n]["EST"]) + ", EST from dominant parent(" + G.node[dominant_parent]["name"] + ")=" + str(
                maxest) + "; deadline=" + str(deadline)
            ...
            print
            "\n**** EST mismatch: " + "EST(" + G.node[n]["name"] + ")=" + str(G.node[n]["EST"]) + " < " + "EST(" + \
            G.node[dominant_parent]["name"] + ")=" + str(maxest)

            return -1
        elif G.node[n]["EST"] > deadline:

            print
            "\n**** Wrong EST: " + "EST(" + G.node[n]["name"] + ")=" + str(G.node[n]["EST"]) + "> deadline=" + str(
                deadline)
...
def graphCheckLFT():
    for n in xrange(0, number_of_nodes):
        ...
        if minlft < 0:
            ...
            print
            "\n**** Negative LFT : " + "LFT(" + G.node[n]["name"] + ")=" + str(
                G.node[n]["LFT"]) + " LFT from dominant child(" + G.node[dominant_child]["name"] + ")=" + str(minlft)
            ...
            print
            "\n**** LFT mismatch: " + "LFT(" + G.node[n]["name"] + ")=" + str(G.node[n]["LFT"]) + " > " + "LFT(" + \
            G.node[dominant_child]["name"] + ")=" + str(minlft)
    ...
    for i in xrange(0, len(instances)):
        if len(instances[i]) > 1:
            for j in xrange(0, len(instances[i]) - 1):
                ...
                if idle_time > 0:
                    tot_idle += idle_time
                    idles += "\n Instance[" + str(i) + "] constains idle time: " + "EST(" + G.node[instances[i][j + 1]][
                        "name"] + ")-EFT(" + G.node[instances[i][j]]["name"] + ")>0"
    print
    idles
    return tot_idle
            ...
            for j in xrange(0, len(instances[i]) - 1):
                ...
                if idle_time > 0:
                    if idle_time >= comm_time:
                        ...
                        print
                        "split instance(" + str(i) + ") at index " + str(j + 1) + " idle_time=" + str(
                            idle_time) + " througput=" + str(comm_time)
                        # adjust throughput
...
def graphCalcEFT(d):
    ...
    if verbose > 1:
        print
        "predecessors(" + str(d) + "):", predecessors
    for p in predecessors:
        ...
        if verbose > 1:
            print
            "a) graphCalcEFT( " + str(p) + " )"
        est = graphCalcEFT(p)
        if verbose > 1:
            print
            "a) est=" + str(est) + " <-graphCalcEFT( " + str(p) + " )"
    ...
    if verbose > 1:
        print
        G.node[d]["name"] + ": EST=" + str(G.node[d]["EST"]), ",", "EFT=" + str(G.node[d]["EFT"])
...
def graphCalcLST(d):
    ...
    for c in successors:
        ...
        if verbose > 1:
            print
            "a) graphCalcLST( " + str(c) + " )"
        lft = graphCalcLST(c)
        if verbose > 1:
            print
            "a) lft=" + str(lft) + " <-graphCalcLST( " + str(c) + " )"
    ...
    if verbose > 1:
        print
        G.node[d]["name"] + ": EST=" + str(G.node[d]["LST"]), ",", "EFT=" + str(G.node[d]["LFT"])
...
def assignPath(p):
    ...
    if len(p) == 0:
        print
        "Zero path len: no assignment possible"
        return -1
    ...
    if prop_cas == 0:
        if verbose > 0:
            print
            "no pre assignment found for: ", p_str
    elif prop_cas > 0:
        p_cas = prop_cas
        if verbose > 0:
            print
            "pre assignment " + str(p_cas) + " for path ", p_str
            ...
            if (p[0] in G.successors(instances[i][inst_len - 1])) or (p[p_len - 1] in G.predecessors(instances[i][0])):
                ...
                if prop_cas > 0:
                    ...
                    if p_cas == -1:
                        if verbose > 0:
                            print
                            "check extended instance", new_inst, "with prop_cas=" + str(prop_cas)
                            print
                            "new_inst_cost=" + str(new_inst_cost), "<", "best_inst_cost=" + str(best_inst_cost) + "?:",
                        ...
                        if verbose > 0:
                            ...
                            "check extended instance", new_inst, "with prop_cas=" + str(prop_cas)
                            print
                            "new_inst_cost=" + str(new_inst_cost), "<=", "(inst_cost=" + str(
                                inst_cost) + "+p_cost=" + str(p_cost) + ")?:",
                        if new_inst_cost <= (inst_cost + p_cost):
                            ...
                            if verbose > 0:
                                print
                                "new_inst_cost=" + str(new_inst_cost), "<", "best_inst_cost=" + str(
                                    best_inst_cost) + "?:",
                            if new_inst_cost < best_inst_cost:
                                best_inst = i
                                best_inst_cas = prop_cas
    ...
    if best_inst >= 0:
        ...
        if best_inst_cas != old_inst_cas:
            ...
            if verbose > 0:
                print
                "adjustInstanceAttributes due to new assignment of instance", str(best_inst)
            adjustInstanceAttributes()
        ...
        if G.node[p[0]]["EST"] >= G.node[instances[best_inst][best_inst_len - 1]]["EFT"]:
            if verbose > 0:
                print
                "add path at end of old instance " + str(best_inst), "of Service " + str(best_inst_cas)
            for j in xrange(0, p_len):
            ...
            if verbose > 0:
                print
                "add path at begin of old instance " + str(best_inst), "of Service " + str(best_inst_cas)
            for j in xrange(0, p_len):
                k = p_len - 1 - j
        ...
        if verbose > 0:
            print
            "add path to new instance ", str(len(instances)), "of Service " + str(p_cas)
        inst_num = -1
        new_instance = []
        for i in p:
        ...
        print
        "assignment failed for path", p_str
        return -1
...
def getCheapestAssignment(p):
    ...
    if new_best_cas > 0:
        if verbose > 0:
            print
            "proposal assignment " + str(new_best_cas) + "(" + str(new_instance_time) + ") for " + p_str
    else:
        if verbose > 0:
            print
            "proposal assignment " + str(new_best_cas) + " for " + p_str

    if verbose > 0:
        print
        "cheapest assignment " + str(new_best_cas) + " for " + p_str
...
def checkClusterLimits(l, p):
    ...
    if verbose > 0:
        print
        "checkClusterLimits for service " + str(l)
    ...
    for i in xrange(0, len(p)):
        ...
        if verbose > 0:
            print
            "R1: p[" + str(i) + "]=" + G.node[p[i]]["name"] + ": inst_eft=" + str(inst_eft) + ">LFT(" + str(
                G.node[p[i]]["name"]) + ")=" + str(G.node[p[i]]["LFT"]) + "?",
    ...
    for i in xrange(0, len(p)):
        ...
        if verbose > 0:
            print
            "R2 p[" + str(i) + "]=" + G.node[p[i]]["name"] + ": (p[0].EST=" + str(
                G.node[p[0]]["EST"]) + "+inst_time(" + str(l) + ")=" + str(incr_inst_time) + ") < est_limits[" + str(
                i) + "]=" + str(est_limits[i]) + "(parent " + G.node[dominant_parents[i]]["name"] + ")?",
    ...
    for i in xrange(0, len(p)):
        ...
        if verbose > 0:
            print
            "R3 p[" + str(j) + "]=" + G.node[p[j]]["name"] + ": lft_p=" + str(lft_p) + "-inst_time(" + str(
                l) + ")=" + str(incr_inst_time) + " >lft_limits[" + str(j) + "]=" + str(lft_limits[j]) + "(child " + \
            G.node[dominant_childs[j]]["name"] + ")?",
...
def printResult():
    ...
    if len(instances) > 0:
        ...
        print
        "\n(#,<>)", "S1:(" + str(nS1) + "," + str(round(m1, 2)) + ")", "S2:(" + str(nS2) + "," + str(
            round(m2, 2)) + ")", "S3:(" + str(nS3) + "," + str(round(m3, 2)) + ")"
        print
        "\n\t" + str(total_cost) + "\t" + str(G.node[number_of_nodes - 1]["EFT"]) + "\t(" + str(nS1) + "," + str(
            round(m1, 2)) + ")\t(" + str(nS2) + "," + str(round(m2, 2)) + ")\t(" + str(nS3) + "," + str(
            round(m3, 2)) + ")"
    else:
        print
        "**** No instances found ****"
...
def main(argv):
    ...
    if (G.node[0]["time1"] == 0 and G.node[0]["time2"] == 0 and G.node[0]["time3"] == 0):
        G.node[0]["time"] = 0
        G.node[0]["assigned"] = 1
    if (G.node[number_of_nodes - 1]["time1"] == 0 and G.node[number_of_nodes - 1]["time2"] == 0 and
            G.node[number_of_nodes - 1]["time3"] == 0):
    ...
    print
    "sum time1: ", str(sum_time1)
    print
    "sum time2: ", str(sum_time2)
    print
    "sum time3: ", str(sum_time3)
    ...
    print
    "critical path: ", pcp, " mean_indegree:" + str(round(mean_indegree, 2))
    print
    "criticalp_time(S1)=" + str(criticalp_time1) + " is " + str(round(critper1, 2)) + "% of deadline(" + str(
        deadline) + ")"
    print
    "criticalp_time(S2)=" + str(criticalp_time2) + " is " + str(round(critper2, 2)) + "% of deadline(" + str(
        deadline) + ")"
    print
    "criticalp_time(S3)=" + str(criticalp_time3) + " is " + str(round(critper3, 2)) + "% of deadline(" + str(
        deadline) + ")"
    print
    "criticali_time(S1)=" + str(criticali_time1) + " is " + str(round(critreduc1, 2)) + "% of deadline(" + str(
        deadline) + ")"
    print
    "criticali_time(S2)=" + str(criticali_time2) + " is " + str(round(critreduc2, 2)) + "% of deadline(" + str(
        deadline) + ")"
    print
    "criticali_time(S3)=" + str(criticali_time3) + " is " + str(round(critreduc3, 2)) + "% of deadline(" + str(
        deadline) + ")"

    start_str = "start configuartion: cost=" + str(sum_time1 * prices[0]) + "  EFT(exit)=" + str(
        G.node[(number_of_nodes - 1)]["EFT"])
    ...
    if retVal == -1:
        ...
        # printResult()
        print
        "final configuration:", critical_str
        print
        "final configuration: cost=" + str(total_cost) + "  EFT(exit)=" + str(G.node[(number_of_nodes - 1)]["EFT"])
