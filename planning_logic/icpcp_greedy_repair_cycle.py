            ...
            for j in range(0, len(instances[i]) - 1):
                ...
                if idle_time > 0:
                    if idle_time >= comm_time:
                        ...
                        print("split instance(" + str(i) + ") at index " + str(j + 1) + " idle_time=" + str(
                ...
                if prop_cas > 0:
                    ...
                    if p_cas == -1:
                        if verbose > 0:
                            ...
                            print("new_inst_cost=" + str(new_inst_cost), "<",
                                  "best_inst_cost=" + str(best_inst_cost) + "?:", end=' ')
                        ...
                        if verbose > 0:
                            print("check extended instance", new_inst, "with prop_cas=" + str(prop_cas))
                            print("new_inst_cost=" + str(new_inst_cost), "<=",
                                  "(inst_cost=" + str(inst_cost) + "+p_cost=" + str(p_cost) + ")?:", end=' ')
...
def checkClusterLimits(l, p):
    ...
    for i in range(0, len(p)):
        ...
        if verbose > 0:
            print("R1: p[" + str(i) + "]=" + G.node[p[i]]["name"] + ": inst_eft=" + str(inst_eft) + ">LFT(" + str(
                G.node[p[i]]["name"]) + ")=" + str(G.node[p[i]]["LFT"]) + "?", end=' ')
    ...
    for i in range(0, len(p)):
        ...
        if verbose > 0:
            print("R2 p[" + str(i) + "]=" + G.node[p[i]]["name"] + ": (p[0].EST=" + str(
                G.node[p[0]]["EST"]) + "+inst_time(" + str(l) + ")=" + str(incr_inst_time) + ") < est_limits[" + str(
                i) + "]=" + str(est_limits[i]) + "(parent " + G.node[dominant_parents[i]]["name"] + ")?", end=' ')
    ...
    for i in range(0, len(p)):
        j = len(p) - 1 - i
        if verbose > 0:
            print("R3 p[" + str(j) + "]=" + G.node[p[j]]["name"] + ": lft_p=" + str(lft_p) + "-inst_time(" + str(
                l) + ")=" + str(incr_inst_time) + " >lft_limits[" + str(j) + "]=" + str(lft_limits[j]) + "(child " +
                  G.node[dominant_childs[j]]["name"] + ")?", end=' ')
...
def printResult():
    ...
    if len(instances) > 0:
        ...
        if (nodes_in_inst != number_of_nodes):
            ...
            for j in range(0, number_of_nodes):
                if nonp[j] == 0:
                    ...
                    tot_non_inst += 1
            print("\nnon instance nodes: n=" + str(tot_non_inst) + " " + nonstr[1:] + " with extra cost: " + str(
                extra_cost))
            total_cost += extra_cost
        tot_idle = checkIdleTime()
        if tot_idle > 0:
            print("\nTotal cost: " + str(total_cost) + " for " + str(number_of_nodes),
                  "nodes with tot idle=" + str(tot_idle))
        else:
        ...
        print("\n(#,<>)", "S1:(" + str(nS1) + "," + str(round(m1, 2)) + ")",
              "S2:(" + str(nS2) + "," + str(round(m2, 2)) + ")", "S3:(" + str(nS3) + "," + str(round(m3, 2)) + ")")
...
def main(argv, command_line=True, graph=None):
    ...
    print("criticalp_time(S1)=" + str(criticalp_time1) + " is " + str(round(critper1, 2)) + "% of deadline(" + str(
        deadline) + ")")
    print("criticalp_time(S2)=" + str(criticalp_time2) + " is " + str(round(critper2, 2)) + "% of deadline(" + str(
        deadline) + ")")
    print("criticalp_time(S3)=" + str(criticalp_time3) + " is " + str(round(critper3, 2)) + "% of deadline(" + str(
        deadline) + ")")
    print("criticali_time(S1)=" + str(criticali_time1) + " is " + str(round(critreduc1, 2)) + "% of deadline(" + str(
        deadline) + ")")
    print("criticali_time(S2)=" + str(criticali_time2) + " is " + str(round(critreduc2, 2)) + "% of deadline(" + str(
        deadline) + ")")
    print("criticali_time(S3)=" + str(criticali_time3) + " is " + str(round(critreduc3, 2)) + "% of deadline(" + str(
        deadline) + ")")
